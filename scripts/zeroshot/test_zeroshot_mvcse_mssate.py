...
评估预训练的 MVCSE-MSSATE 模型在各个数据集上的 zero-shot 分类性能。
...
使用方法:
CUDA_VISIBLE_DEVICES=0 python test_zeroshot_mvcse_mssate.py \
...
def load_model(ckpt_path: str):
    """加载预训练的 MVCSE-MSSATE 模型"""
    print(f"Loading model from: {ckpt_path}")
    model = MVCSEMSSATEModel.load_from_checkpoint(ckpt_path)
...
@torch.no_grad()
def get_class_embeddings(model, class_names: List[str], fusion_mode: str = 'concat'):
    """
    计算类别的文本嵌入

    Args:
        model: MVCSE-MSSATE 模型
        class_names: 类别名称列表
        fusion_mode: 多尺度融合方式 ('concat', 'mean', 'rhythm')
    ...
    for text in tqdm(class_names, desc='Computing class embeddings'):
        text = text.lower()
        texts = [text]
        tokenized = model._tokenize(texts)

        # 获取文本嵌入
        class_embedding = model.get_text_emb(
...
@torch.no_grad()
def get_ecg_predictions(model, loader, zeroshot_weights, fusion_mode: str = 'concat'):
    """
    计算 ECG 的预测结果
    ...
    for batch in tqdm(loader, desc='Computing ECG embeddings'):
        ...
        # 获取 ECG 嵌入
        ecg_emb = model.ext_ecg_emb(ecg, mode=fusion_mode)
...
def zeroshot_eval(
    ...
    # 计算类别嵌入
    zeroshot_weights = get_class_embeddings(model, target_prompts, fusion_mode)

    # 计算预测
    pred = get_ecg_predictions(model, test_loader, zeroshot_weights, fusion_mode)
...
def main(args):
    ...
    if not model.use_multiscale and fusion_mode != 'concat':
        print(f"Warning: Single-scale model detected, fusion_mode '{fusion_mode}' ignored.")
        fusion_mode = 'concat'  # 单尺度模型不需要融合
    ...
    exp_name = f"zeroshot_mvcse_mssate_{model.ecg_encoder_name}_{extension}"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="MVCSE-MSSATE Zero-shot Evaluation")
    ...
    parser.add_argument('--fusion_mode', type=str, default='concat',
                        ...
                        help='Multi-scale fusion mode (for hierarchical models)')
