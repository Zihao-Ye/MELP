...
使用方法:
CUDA_VISIBLE_DEVICES=0 python main_finetune_mvcse_mssate.py \
...
def main(hparams: Namespace):
    ...
    extension = f"mvcse_mssate_finetune_{hparams.dataset_name}_{hparams.finetune_mode}_{extension}"

    ckpt_dir = os.path.expanduser(
        f"~/autodl-tmp/logs/mvcse_mssate_finetune/ckpts/{extension}")
    ...
    logger_dir = os.path.expanduser("~/autodl-tmp/logs/mvcse_mssate_finetune")
    os.makedirs(logger_dir, exist_ok=True)

    wandb_logger = WandbLogger(
        project="mvcse_mssate_finetune",
        save_dir=logger_dir,
        name=extension
    )
    ...
    if hparams.ckpt_path:
        ...
        pretrain_model = MVCSEMSSATEModel.load_from_checkpoint(hparams.ckpt_path)
    else:
        print("No checkpoint provided, using randomly initialized model")
        pretrain_model = MVCSEMSSATEModel(
...
class MVCSEMSSATEFineTuner(SSLFineTuner):
    """
    MVCSE-MSSATE全微调模型
...
if __name__ == '__main__':
    parser = ArgumentParser(description="Finetune MVCSE-MSSATE Model.")
    ...
    parser.add_argument("--lead_group_strategy", type=str, default="none",
                        ...
                        help="Lead grouping strategy (used when no ckpt_path provided): none, limb_chest (2 groups), lisa (5 groups)")
